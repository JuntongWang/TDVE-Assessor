...
if __name__ == '__main__':
    ...
    with open('../../datasets/video_mos1_test.json', 'r') as f:
        datas = json.load(f)
    ...
    for data in tqdm(datas):
        labels_list.append(float(data["messages"][1]["content"]))
        ...
        pred_mos_list.append(float(reply))
        eval_info[data["videos"]] = float(reply)


    print("------------------------------ MOS1 ------------------------------")
    SRCC = stats.spearmanr(pred_mos_list, labels_list)[0] 
    PLCC = stats.pearsonr(pred_mos_list, labels_list)[0] 
    KRCC = stats.kendalltau(pred_mos_list, labels_list)[0]

    print("SRCC: ", SRCC)
    print("PLCC: ", PLCC)
    print("KRCC: ", KRCC)

    with open("mos1_eval_info.json", 'w') as f:
